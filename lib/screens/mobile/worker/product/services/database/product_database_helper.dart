...
class ProductDatabaseHelper {
  static const String PRODUCTS1_COLLECTION_NAME = "products1";
  ...
  Future<List<String>> searchInProducts(String query,
    ...
    Query queryRef;
    if (productType == null) {
      queryRef = firestore.collection(PRODUCTS1_COLLECTION_NAME);
    } else {
      final productTypeStr = EnumToString.convertToString(productType);
      print(productTypeStr);
      queryRef = firestore
          .collection(PRODUCTS1_COLLECTION_NAME)
  ...
  Future<bool> addUsersRatingForProduct(String productId, int rating,
      {int oldRating}) async {
    final productDocRef =
        firestore.collection(PRODUCTS1_COLLECTION_NAME).doc(productId);
    final ratingsCount =
  ...
  Future<String> addUsersProduct(Product product) async {
    ...
    final productsCollectionReference =
        firestore.collection(PRODUCTS1_COLLECTION_NAME);
  ...
  Future<List<String>> get allProductsList async {
    final products = await firestore.collection(PRODUCTS1_COLLECTION_NAME).get();
    List productsId = List<String>();
